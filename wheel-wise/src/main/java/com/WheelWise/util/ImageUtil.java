...
public class ImageUtil {
	...
	public boolean uploadImage(Part part, String rootPath, String saveFolder) {
		// Determine the save path based on the root path and save folder
		...
		try {
			// Get the image name from the uploaded part
			String imageName = getImageNameFromPart(part);
			// Create the file path for saving the image
			String filePath = savePath + "/" + imageName;

			// Check if the file already exists, and if so, modify the filename to avoid
			// overwriting
			...
			// Write the uploaded image file to the server
			part.write(filePath);
